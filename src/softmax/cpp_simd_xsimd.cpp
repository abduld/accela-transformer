...
static void CPP_XSIMD(benchmark::State &state) {
  ...
  std::vector<float, xsimd::aligned_allocator<float, XSIMD_DEFAULT_ALIGNMENT>>
      in(N, 1), out(N);
  ...
  for (auto _ : state) {
    float maxVal = xsimd::reduce(inData, inData + N, inData[0], [=](const auto &x,const auto &y) { return xsimd::max(x,y); });
