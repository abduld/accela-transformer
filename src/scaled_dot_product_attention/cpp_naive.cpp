...
static void BENCHMARK_NAME(CPP_Naive_BatchFirst)(benchmark::State& state) {
  std::vector<float, xsimd::aligned_allocator<float, XSIMD_DEFAULT_ALIGNMENT>> Q(BATCH_SIZE * N,
                                                                                  1),
      K(BATCH_SIZE * N), ;
  for (auto _ : state) {
    for (int ii = 0; ii < BATCH_SIZE; ii++) {
      ...
      cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, M, N, K, alpha, aData, lda, bData, ldb,
                  beta, cData, ldc);
